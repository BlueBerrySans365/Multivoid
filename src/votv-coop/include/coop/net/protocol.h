...
enum class ReliableKind : uint8_t {
    ...
    AtvOccupied = 126,  // v138 (atv_sync single-driver): HOST->CLIENT rejection when a
                        //     second player tries to sit in an occupied ATV. The host sends
...
// v138 (2026-08-02): ATV single-driver enforcement -- HOST->CLIENT rejection event.
// Sent when a second player tries to sit in an ATV that is already occupied. The payload
// carries the ATV key + the occupying player's peer slot so the client can show a toast
// "ATV occupied by [name]" using the roster registry. One-shot, fire-and-forget.
// coop/atv_sync.
struct AtvOccupiedPayload {
    WireKey key;          // 32 -- the ATV's Key@0x0618
    uint8_t driverSlot;   // 1  -- the peer slot of the current driver (0xFF = unknown)
