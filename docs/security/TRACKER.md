...
>
> **As of 2026-07-24:** **A11** added and **BUILT** in the same day (`b2c4b3ef`) — the origin VPS IP was
> sitting in four tracked docs on the public repo, defeating the root domain's own Cloudflare proxying.
> Found by the pre-push leak audit, not by an audit agent. Still 16 OPEN (A11 shipped immediately).
> Two framing notes worth keeping: the HOSTNAME is public by necessity and hiding it would be theatre,
> and history was deliberately not rewritten — so this is casual-discoverability removal, not secrecy.
>
> **As of 2026-08-06:** **W8** → **BUILT** (`0bc5c2ea` — floppybox hard reject at >15 entries, laptop_buffer
> append cap at >=1024 per array; not hostile-drilled, per-array residual noted in the row). **A3** →
> **PARTIAL** (`holder_table` authority gate shipped 2026-08-01 — covers keyed-Aprop destroy/convert on
> the host-apply path; named residuals in the row and the design note).
>
> **As of 2026-08-06 (2):** **W8 residual** — laptop_buffer cap changed from per-array to combined
> (`q.data.size() + q.buffer.size() >= 1024`), closing the 2× residual. **A3b** → **DEFERRED**
> (disconnect race: `ClearSlot` runs before reliable-inbox drain in `event_feed::Update`; real but narrow
> + safe direction, see the row + investigation doc).
>
> **As of 2026-08-08:** **A5** → **BUILT** — BalanceDelta lane fully retired: `OnDeltaRequest` removed,
> `CreditRouted` client branch removed, dispatch drops on receipt. Dead code cleaned per RULE 2.
...
| **A3** | **`PropDestroy` / `PropConvert` / `PropRelease` trust any client's assertion about any entity** — only an eid-range check. The holder table exists and is consulted correctly in `trash_grab_intent.cpp:298-302`, but not here. Aggravating: these kinds are client-relayable, so a forged destroy is **fanned out to other clients before the host validates it** | `[A]` `event_dispatch_entity.cpp:253,316,73`, ranges `:286-292,348-363`; relay `session_lanes.h:181-185`, `session.cpp:454` vs `:463` | Any peer → **irreversibly wipes a stranger's persistent world** | `PLAN_03` §2 | **PARTIAL** — `holder_table` (eid→slot authority gate) shipped 2026-08-01: `remote_prop_destroy.cpp:170` and `remote_prop_convert.cpp:65` now reject if `senderSlot` does not hold the entity. **Covers:** keyed-Aprop destroy/convert on the host-apply path. **Does NOT cover:** trash proxies (separate lifecycle, explicitly skipped), host-originated ops (senderSlot<0 bypass), relay-whitelist fan-out timing (whether a forged destroy is relayed to other clients before host denial is unverified), non-prop entities (NPC/world-actor destroys don't flow through holder_table). Design note: `research/findings/props-lifecycle/votv-holder-table-authority-design-2026-08-06.md` |
| **A3b** | **`holder_table::ClearSlot` runs before the reliable inbox is drained on disconnect.** `event_feed::Update` calls `ReconcileFromSession` (→ `ClearRow` → `Transition` → `DisconnectSlot` → `OnGrabHolderLeft` → `ClearSlot`) BEFORE draining `reliableInbox_`. A departing peer's in-flight `PropDestroy` then hits `IsHeldBy` → false → DENIED (legitimate destroy dropped) | `[V]` investigated 2026-08-06: `subsystems.cpp:378` (`OnGrabHolderLeft`), `trash_grab_intent.cpp:378` (`ClearSlot`), `event_feed.cpp:144` (reconcile before drain) | Departing peer → **last destroy dropped (entity lingers); minor divergence, safe direction** | `PLAN_03` §2 | **DEFERRED** — real but narrow + safe direction. The dangerous variant (old peer's stale message reaching a NEW peer reusing the slot) is already covered by the epoch check in `HandleMessage` (`session.cpp:316-348`). The residual is a missed cleanup (entity lingers), NOT corruption. Fix would mean deferring `ClearSlot` to AFTER the inbox drain in `event_feed::Update` (structural to the load-bearing reconcile-then-drain order that Joins rely on). Low priority vs. the blast radius. Investigation: `research/findings/props-lifecycle/votv-taskA-holder-table-thread-safety-connect-race-2026-08-06.md` §A2 |
...
| **A5** | **`BalanceDelta` is an unbounded client-authored economy write.** Length is validated; the value is not | `[A]` `event_feed.cpp:294-306` → `balance_sync.cpp:96-103` | Any peer → **sets the shared balance to ±2³¹** | `PLAN_04` §3 | **BUILT** — BalanceDelta lane retired (2026-08-01): dispatch drops on receipt, `OnDeltaRequest` removed, `CreditRouted` client branch removed. Credits route only through host's AddPoints. |
...
| **W8** | **Inbound append lanes ignore their own caps.** The floppybox 15-entry cap is a `UE_LOGW` that does not reject; laptop buffer `appendTail` has no cap | `[A]` `floppybox_sync.cpp:398-403`; `laptop_buffer_sync.cpp:313-316` | Any peer → unbounded engine-array growth | `PLAN_02` §4 | **BUILT** `0bc5c2ea` — floppybox now rejects + `pop_back` when >15 entries (hard reject, no longer a warning); laptop_buffer `HostApplyBatch` rejects append when `arr.size() >= 1024`. Both cited surfaces capped. **Not hostile-drilled:** a sustained append-spam against either lane has not been measured. **Residual FIXED 2026-08-06:** the laptop cap is now a combined cap across both arrays (`q.data.size() + q.buffer.size() >= 1024`), so a client can't grow one array to deflect the limit and keep growing the other. Total entries across both arrays <= 1024. |
...
| **W12** | **Remote rasterisation amplification.** The ImGui 1.92 flip (2026-07-30) makes every codepoint in the 7,258-strong repertoire rasterisable ON DEMAND from chat text -- the one attacker-controlled string in the process. A few hundred bytes of deliberately diverse UTF-8 forces thousands of FreeType rasterisations on EVERY receiving peer, inside the frame that draws them. RHI-independent. The existing caps (`kMaxSnapshotLines` 206 x `text[256]`) bound ONE FRAME, not a stream of fresh messages, and nothing in ImGui bounds per-frame rasterisation (measured by absence) | `[V]` the cost side is measured: at a starved `TexMax=64` an i18n load produced 79 failed packs on one client. The capacity side is measured too -- the pathological demand (feed 18px + bubble 14.08px + scoreboard 16px, ALL asking for the whole repertoire) is 3,589,892 px2 against 2048^2's 4,194,304, i.e. **0.856x: it fits, with 17% headroom, not the 3x an earlier revision claimed** | Any peer -> a render-thread stutter on every other peer; pack exhaustion would be a PERMANENT box (a baked drawn every frame is never discarded) | Bounded at the RECEIVE boundary, not in a draw loop -- three surfaces rasterise remote text, so a draw-time cap is a site list. `coop/text/novelty_ledger` caps never-before-seen codepoints per peer per window (`net.novelty_budget` 512 / `net.novelty_window_ms` 10000) and refuses the whole field, exactly as the strict decode does. It bounds the BURST only -- the ledger is monotone, so it does NOT prevent exhaustion; capacity does, with `ui/atlas_watch.cpp`'s pack-failure detector as the tripwire | **BUILT** (selftest 6/6, both detectors shown RED by drill; not hands-on) |
...
| **A11** | **The origin VPS IP sat in four tracked docs on the PUBLIC repo, defeating the root domain's Cloudflare proxying.** `MULTIPLAYER_UI.md` itself records that the root of `multivoid.dev` is *proxied* through Cloudflare to that address — the whole point of which is to hide the origin — while that same file, `ROADMAP.md`, and two `research/findings/network/` docs printed the raw IP. Standard origin-bypass exposure, and it names the hosting provider. **Distinguish carefully from the hostname:** `master.multivoid.dev` is public BY NECESSITY (`protocol.h:1113` compiles it into a DLL we distribute), so hiding *it* would be theatre; the origin IP is the part meant to stay movable and shieldable. Note the coupling too — while the IP was published, moving or proxying the box later would have bought nothing until the docs were cleaned | `[V]` verified 2026-07-24 by the pre-push leak audit: `git grep -l` against `origin/main` returned `docs/MULTIPLAYER_UI.md`, `docs/ROADMAP.md`, `research/findings/network/votv-master-server-RE-and-rust-port-scope-2026-07-16.md`, `research/findings/network/votv-tls-tier-b-c-DESIGN-2026-07-20.md`; the proxying claim is `MULTIPLAYER_UI.md:286` | Scraper / drive-by reader → **bypasses Cloudflare straight to the origin**, and learns the host | — | **BUILT** `b2c4b3ef` — 10 occurrences replaced with `<coop-vps>`; a standing "never write the origin IP into tracked files" rule added to the session-loaded memory index. **History deliberately NOT rewritten** (user decision): a force-push on a public repo with distributed clones costs more than it buys for a value DNS hands out. This removes *casual* discoverability going forward and is explicitly NOT a claim that the address is now secret |
